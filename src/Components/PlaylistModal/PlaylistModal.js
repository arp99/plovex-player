...
export const PlaylistModal = ({ showModal, videoData }) => {
    ...
    if (playlistName !== "") {
      try {
        const response = await axios.post(
          "https://plovex-player-backend-production.up.railway.app/playlists",
          {
            playlistName,
  ...
  const isVideoInPlaylist = (playlistId) => {
    const playlistToSearch = playlist.find(
      (playlist) => playlist._id === playlistId
    );
    const isVideoPresent = playlistToSearch.videos.find(
  ...
  const toggleVideoInPlaylist = async (playlistId) => {
    try {
      await axios.post(
        `https://plovex-player-backend-production.up.railway.app/playlists/${playlistId}`,
        {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <small id={`${PlaylistStyle.close}`} onClick={() => showModal(false)}>
          X
        </small>
      </div>
      ...
      {state ? (
        ...
        >
          <small id={`${PlaylistStyle.create}`} onClick={() => setState(true)}>
            +
          </small>
