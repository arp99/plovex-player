...
export const Thumbnail = ({ videoData }) => {
  ...
  const addToHistoryHandler = async () => {
    try {
      await axios.post(
        "https://plovex-player-backend-production.up.railway.app/history",
        {
          _id: videoData._id,
        }
      );
  ...
  return (
    <div className={`${ThumbnailStyle.card}`} onClick={addToHistoryHandler}>
      ...
      <div className={`${ThumbnailStyle["card-details"]}`}>
        <h2>
          <Link
            to={`/v/${videoId}`}
            className={
              theme === "dark" ? `${ThumbnailStyle.dark_theme_link}` : ""
            }
          >
            {title}
          </Link>
